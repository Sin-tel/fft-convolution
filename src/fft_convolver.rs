...
mod ipp_fft {
    ...
    fn allocate_aligned_buffer(size: usize) -> Vec<u8> {
        ...
        unsafe {
            ...
            // Create a vector that will free the memory when dropped
        ...
        fn drop(&mut self) {
            unsafe {
                ...
                if !self.scratch_buffer.is_empty() {
                    ...
                    let len = self.scratch_buffer.len();
...
#[cfg(not(feature = "ipp"))]
pub use rust_fft::{complex_multiply_accumulate, complex_size, sum, FFTConvolver, Fft};

#[cfg(feature = "ipp")]
pub use self::ipp_fft::FFT;
